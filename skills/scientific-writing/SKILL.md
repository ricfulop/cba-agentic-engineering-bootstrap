...
## Figures (optional graphical abstract; no AI-art mandate)

A graphical abstract is **optional**. Include one only when the venue asks for
it or the author asks for it. Do not invent one to satisfy this skill. Do not
generate AI figures (scientific-schematics, generate-image, Nano Banana) for a
journal manuscript. Those skills are for board slides and early review
cartoons. Final figures are real data renderings.

**When a figure is warranted, prefer data.** In CBA research trees using
this plugin, use `lib/styles.py` via the `figure-style` skill: `apply_aps_style()` /
`apply_nature_style()` / `apply_science_style()`, `OKABE_ITO` or `MATERIALS`,
and the size helpers. Do not start from K-Dense `style_presets` or a bare
`plt.figure()`. Captions live in LaTeX, not inside the image.

If the venue wants a graphical abstract, it is a **data figure** (measured
result, workflow of real instruments, or a restyled schematic drawn from the
paper's own numbers), landscape, one message, minimal labels. It is not an
AI-generated collage.

There is no minimum figure count. A methods paper with two honest plots beats
a paper padded to "5-8 figures."

---

## Prose register (mandatory)

Every manuscript this skill produces is checked, not styled by eye. The
canonical rule is `manuscript-prose-style` at
`~/.cursor/skills/manuscript-prose-style/SKILL.md`. Restated here because this
skill used to omit it:

| rule | how |
|---|---|
| American English unless the venue says otherwise | Mask TeX, math, bibliography, and control sequences first. `\metre` is a macro. Quoted titles keep their published spelling |
| No en dash or em dash | In `.tex`, use `--` and `---`. In Markdown or plain text, rewrite as a comma, colon, or parentheses. Do not paste U+2013 or U+2014 |
| Does not read as machine-generated | No formulaic tricolon, "not X, but Y" emphasis, "it is worth noting," "paves the way," or the delve/leverage/seamless/pivotal set. Paragraphs develop an argument |
| No watermarking | Zero U+200B/U+FEFF/U+00AD and the rest of the invisible set. No "generated with" comments. No AI attribution trailer in commits |

**Flags, do not silently rewrite.** Run the greps in `manuscript-prose-style`
§§ 3.1-3.4, report hits with line numbers, and wait for a pass. A language
pass that moves a `\SI`, `\label`, `\cite`, or `\ref` has broken the paper.

Noun-phrase headings. Venue house style overrides the default
(APS *Acknowledgments*, Nature *Acknowledgements*).
...
1. Gather literature. Use scite (if configured) to verify citations
   that are already in the draft, and `paper-lookup` for physics/materials
   discovery. `research-lookup` (Parallel / Perplexity) is optional and is
   not the citation oracle. Do not loop "until all citations pass."
...
5. Verify citations once (Literature / scite, or a single verify pass). Accept
   the report. Do not re-run until every DOI is green
6. Check word counts for each section
7. Run the prose register: American English, no en/em dashes, no AI phrasing,
   no watermark characters or generation comments (`manuscript-prose-style`)
