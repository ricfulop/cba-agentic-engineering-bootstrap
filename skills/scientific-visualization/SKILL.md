...
## CBA / this plugin override

In CBA research trees using this plugin, **this skill's `style_presets` is
not the style source.** `lib/styles.py` shipped with this plugin is. Read
`figure-style` and the figure mandate first.

| this skill says | do this instead |
|---|---|
| `from style_presets import style_context` | `from styles import apply_aps_style, apply_nature_style, apply_science_style, OKABE_ITO` |
| `plt.subplots(figsize=(89/25.4, …))` | `aps_single()`, `aps_double()`, `nature_single()`, `science_triple()`, never a raw `figsize` |
| `bbox_inches=None` "preserves page dimensions" | It does not, once `apply_*_style()` has set `savefig.bbox` to `"tight"`. Matplotlib treats `None` as "use the rcParam." The working form is `with plt.rc_context({"savefig.bbox": "standard"}): savefig(...)`. Measure the delivered file in millimeters |

Integrity rules below (honest encoding, no silent missing-data, provenance)
still apply. The export helper may be used **after** a `styles.py` style
function has been applied. Do not let this skill's Okabe-Ito copy drift from
